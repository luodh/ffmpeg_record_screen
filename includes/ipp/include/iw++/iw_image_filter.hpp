...
// Auxiliary parameters class
// C API descriptions has more details
class IwiFilterSepParams: public ::IwiFilterSepParams
{
public:
    IW_BASE_PARAMS_CONSTRUCTORS(IwiFilterSepParams, iwiFilterSep_SetDefaultParams)
    IwiFilterSepParams(double _divisor = 0, int _offset = 0)
    {
        this->divisor   = _divisor;
        this->offset    = _offset;
    }
...
// Performs filtration of an image with an arbitrary separate kernels for rows and for columns
// Throws:
//      ippStsBorderErr                     border value is illegal
//      ippStsDataTypeErr                   data type is illegal
//      ippStsNumChannelsErr                channels value is illegal
...
// Returns:
//      ippStsNoErr                         no errors
IW_DECL_CPP(IppStatus) iwiFilterSep(
    const IwiImage             &srcImage,                           // [in]     Reference to the source image
    IwiImage                   &dstImage,                           // [in,out] Reference to the destination image
    const IwiImage             &kernelX,                            // [in]     Reference to the row filter kernel image. Kernel must have height 1, [16s,32f], [C1]
    const IwiImage             &kernelY,                            // [in]     Reference to the column filter kernel image. Kernel must have height 1, [16s,32f], [C1]
    const IwiFilterSepParams   &auxParams   = IwiFilterSepParams(), // [in]     Reference to the auxiliary parameters structure
    const IwiBorderType        &border      = ippBorderRepl,        // [in]     Extrapolation algorithm and value for out of image pixels: ippBorderConst, ippBorderRepl, ippBorderMirror, ippBorderInMem
    const IwiTile              &tile        = IwiTile()             // [in]     Reference to the IwiTile structure for tiling. By default no tiling is used
)
{
    IppStatus ippStatus = ::iwiFilterSep(&srcImage, &dstImage, &kernelX, &kernelY, &auxParams, border.m_type, border.m_value, &tile);
    OWN_ERROR_CHECK(ippStatus)
    return ippStatus;
}
